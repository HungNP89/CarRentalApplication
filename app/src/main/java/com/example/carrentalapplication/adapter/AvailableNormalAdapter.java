...
public class AvailableNormalAdapter extends RecyclerView.Adapter<AvailableNormalAdapter.ViewHolder> {
    Context context;
    List<AvailableNormalModel> list;

    public AvailableNormalAdapter(Context context, List<AvailableNormalModel> list) {
        this.context = context;
        this.list = list;
    }
    @NonNull
    @Override
    public AvailableNormalAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.card_view_popular_normal, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull AvailableNormalAdapter.ViewHolder holder, int position) {
